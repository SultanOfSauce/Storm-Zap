...
def broadcastMessage(conn: socket.socket, message: str, a_list: List[socket.socket], username: str):
    '''
    Small function to send a message to all the people connected to the server, except the client who is sending the message.
    '''
    for c in a_list:
        if c != conn:
            c.sendall(messageBroadcast(username, message, False).encode('ascii'))
    return
